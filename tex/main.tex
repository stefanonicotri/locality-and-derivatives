...
\subsection{Why Doesn't This Apply to a Finite Series?}
A finite series of derivatives is mathematically a convolution as well. The distinction between locality and nonlocality relies entirely on the nature of the convolution kernel.

Consider a finite operator, stopping at the second derivative: $\hat{O} = c_0 + c_1 \partial_x + c_2 \partial_x^2$. Moving to Fourier space, the multiplier $P(i\xi)$ is a finite polynomial:
\[
\mathcal{F}(\hat{O} f(x)) = (c_0 + c_1(i\xi) + c_2(i\xi)^2) \hat{f}(\xi)
\]
To find the real-space kernel $K(x)$, we take the inverse Fourier transform of this polynomial. The inverse Fourier transform of $1$ is the \textbf{Dirac delta function} $\delta(x)$, the inverse transform of $i\xi$ is its derivative $\delta'(x)$, and so on. Thus, the kernel is:
\[
K(x) = c_0 \delta(x) + c_1 \delta'(x) + c_2 \delta''(x)
\]
Setting up the convolution integral over all space gives:
\[
\hat{O} f(x) = \int_{-\infty}^{\infty} \Big( c_0 \delta(x-y) + c_1 \delta'(x-y) + c_2 \delta''(x-y) \Big) f(y) \, dy
\]
By the fundamental property of the Dirac delta function, it is exactly zero everywhere except where $x=y$. Convoluting a function with $\delta(x)$ simply extracts $f(x)$, and convoluting with $\delta'(x)$ extracts $f'(x)$. The infinite bounds instantly collapse into an infinitesimally small local operation:
\[
\hat{O} f(x) = c_0 f(x) + c_1 f'(x) + c_2 f''(x)
\]
Even though the finite series is technically an integral over all space, it multiplies distant points by zero. Infinite series, however, create transcendental functions (like $e^{-\xi^2}$) whose kernels are continuous, extended distributions that never equal zero when $x \neq y$, thereby forcing the integral to physically scan the entire domain.
